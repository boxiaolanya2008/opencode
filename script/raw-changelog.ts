...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/opencode packages/sdk packages/plugin packages/opendestop-next packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      ...
      else if (file.startsWith("packages/opendestop-next/src-tauri/")) areas.add("tauri")
      else if (file.startsWith("packages/opendestop-next/") || file.startsWith("packages/app/")) areas.add("app")
