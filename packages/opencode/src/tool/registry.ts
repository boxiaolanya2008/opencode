...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const codeedit = yield* CodeEditTool
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        ...
        // code_edit 取代 edit + apply_patch，默认对所有模型开启。
        // edit / apply_patch 即使被自定义插件重新注册也默认不暴露，避免模型在两套编辑语义间混乱。
        // write 保留全量写入场景。
        if (tool.id === EditTool.id || tool.id === ApplyPatchTool.id) return false
        if (tool.id === CodeEditTool.id || tool.id === WriteTool.id) return true
